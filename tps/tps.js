...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function parse_data(data) {
    ...
    if (data.curso.tps.length === 0) {
      ...
      alerta.setAttribute(
        "class",
        "mx-auto mt-10 w-1/2 flex items-center p-4 text-xl text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800"
      );
      ...
      for (let i = 0; i < data.curso.tps.length; i++) {
        ...
        item.setAttribute(
          "class",
          "m-4 px-3 py-auto w-42 h- rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
        );
        ...
        tp.setAttribute(
          "class",
          "dark:text-gray-50 text-2xl font-medium tracking-wide text-center leading-tight p-3"
        );
        tp.append(data.curso.tps[i].nombre);

        const boton_desc = document.createElement("button");
        boton_desc.setAttribute(
          "class",
          "text-white font-medium rounded-lg text-sm px-4 py-2 me-2 mb-2 dark:bg-gray-900 dark:hover:bg-gray-800 hover:ring-2 hover:ring-gray-500"
        );
